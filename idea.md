...
The central design tension is _legibility_: a 6³ block hides 4³ = 64 cubes inside itself.
...
- **Deep bonus:** ×1.5 (rounded up) if every cube in the path is an _interior_ cube
...
- Occluded cubes: raycasting only hits cubes whose _current_ opacity ≥ 0.35, so x-ray
...
- The cursor position is announced via an `aria-live` region:
  _"Layer 3, row 2, column 4, letter T, selected."_
...
1. **Layer slicing** (`[` / `]` or a slider): show only layer _k_ on an axis; the rest
...
- **Touch:** 1 finger = orbit _unless_ the gesture starts on a cube and stays under the
...
- _Nudge:_ flash a cube that starts ≥ 3 unfound words (−1 pt).
- _Reveal:_ show one unfound word's path for 1.5 s (−half its value).
...
- Is 26-neighbour adjacency _too_ generous at 6³ (word count explodes)? Maybe scale
  `minWordLength` and default to `edge` mode for N ≥ 5.
- Should a cube be reusable across _different_ words in the same round? (Yes, as in Boggle.)
...
- **Picking is glyph-pixel, not geometry.** `cubes.pickLetter()` tests the pointer
  against the _rendered glyph quads_ in screen space (nearest-to-camera first, plus a
...
- **Two InstancedMeshes instead of per-instance opacity.** `cubes.js` keeps a `tier`
  ...
  with _colour_, not alpha, so it stays smooth. With the shells hidden the tiers now
...
- **Dictionary:** a compact built-in word list (`js/data/words.js`) makes the game
  ...
  gesture too much to be usable, so _every_ drag orbits (right/middle drag pans anywhere,
